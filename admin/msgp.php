...
if (isset($_POST['sendmsg'])){
    ...
    $que = "SELECT partner FROM `reseller`";
    $data = $con->query($que);
    if($data->num_rows > 0){
        ...
        echo "<script>alert('Meassge sent successfully'); window.location='msgp'</script>";
    }else{
        echo "<script>alert('Failed to send message'); window.location='msgp'</script>";
    } 
}
                            ...
                            <div class="table-responsive mb-4 mt-4">
                                <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $flag= 1;
                                        $qe = "SELECT * from notification where flag = '$flag' ORDER BY id DESC LIMIT 10";
                                        $data = $con->query($qe);
                                        $srno = 1;
                                        if($data->num_rows > 0){
                                            while($rows = $data->fetch_assoc()){
