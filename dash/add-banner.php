...
if (isset($_POST['banner'])){
    ...
    if(isset($_FILES['image'])){
      ...
      /*if($display == 2 || $display == 3){
          if($image_width < 301 && $image_height < 301){
              
          }
          else {
              echo "<script>alert('Sorry, Failed To Upload : Height and Width Should be 300 px.'); window.location='add-banner'</script>";
              die();
          }
      }
      else if($display == 1){
          if($image_width == 600 && $image_height == 337){
              
          }
          else {
              echo "<script>alert('Sorry, Failed To Upload : Height 600 px and Width Should be 337 px.'); window.location='add-banner'</script>";
              die();
          }
      }else if($display == 5){
        if($image_width == 490 && $image_height == 583){
            
        }
        else {
            echo "<script>alert('Sorry, Failed To Upload : Height 583px and Width Should be 490px.'); window.location='add-banner'</script>";
            die();
        }
     }*/
      ...
      if($file_size > 2107152){
         $errors[]='File size must be excately 2 MB';

         echo "<script>alert('Sorry, Failed To Upload Banner : File size must be excately 2 MB'); window.location='add-banner'</script>";
      }
      ...
      if(empty($errors)==true){
    	...
    	if(empty($errors)==true){
            ...
            $que = "INSERT INTO `banner`(`image`, `cust`, `display`, `datee`) VALUES ('$rename_done','$userid','$display','$date')";
...
<html lang="en">
<head>
    <title>Add Banners / Images </title>
        ...
        <div id="content" class="main-content">
            <div class="container">
                        ...
                        <div class="col-lg-12 layout-spacing" style="padding: 0px;">
                            ...
                            <p align="justify"><strong><span style="color: darkorange;">Instruction :</span></strong> Max Image Size 2 MB
                            </p>
							<hr style="border: 1px solid;">
                            ...
                            <form method="post" enctype="multipart/form-data">
							    ...
							    <select class="form-control  basic" name="display" id='mySelect'>
									<option value="5">Custom Texture (Recommended: 490x583 pixel)</option>
									<option value="1">Gallery Banner (Recommended: 600x337 pixel)</option>
                                    <option value="2">Profile Photo 1 (Recommended: 300x300 pixel)</option>
                                    <option value="3">Profile Photo 2 (Recommended: 300x300 pixel)</option>
                                    <option value="4">Background Image (Custom Size)</option>
								</select>
