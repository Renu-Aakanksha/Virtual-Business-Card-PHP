...
if (isset($_POST['addpay'])) {
    ...
    if($ccu == 0){
        $user = $_SESSION['user'];
        $qe = "SELECT DISTINCT `email` FROM `customer` WHERE `cust`='$user'";
        $data = $con->query($qe);
        if($data->num_rows > 0){
            ...
            <div class="container">
                        ...
                        <div class="col-lg-12 layout-spacing" style="padding: 0px;">
                            ...
                            <form method="post" enctype="multipart/form-data">
                                ...
                                <p>Select Customer</p>
							    <select class="form-control  basic" name="cu" required="true">
							        
    							        <option value="0">To All Customers</option>
                                        ...
                                        $qew = "SELECT * FROM `customer` WHERE `email`='$nemail'";
                                        $dataw = $con->query($qew);
                                        $rowsw = $dataw->fetch_assoc();
                                        $nname = $rowsw['name'];
                                                
                                                echo '<option value="'.$nemail.'">'.$nname.'</option>';
                                            }
                                        }
                                        ?>
								</select>
