...
if(isset($_GET['se']) && isset($_GET['es'])){
    ...
    if($fse2 and $fes2){
        ...
        if (time() < ($diff + $fse2)){
            include('../dash/db.php');
            $q = "SELECT partner FROM `reseller` WHERE `id`= '$fes2' limit 1";
        	$data = $con->query($q);
        	if ($data->num_rows > 0) {
                ...
                session_start();
                 $_SESSION['partner'] = $fes2;
                 $_SESSION['partner_ref'] = $id;
        	}else{
        	    echo "<script>alert('Invalid: Forget Password URL'); window.location='../';</script>";
        	}
        }else{
            echo "<script>alert('Reset Password Link Experied'); window.location='../';</script>";
        }
...
if (isset($_POST['sub'])) {
    ...
    $part_id = $_SESSION['partner'];

    if ($pass == $pass2) {
        $q_update = "UPDATE `reseller` SET `pass`='$pass2' WHERE id = '$part_id' ";
