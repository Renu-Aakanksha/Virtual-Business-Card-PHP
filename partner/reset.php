...
if(isset($_POST['send'])){
    ...
    $q = "SELECT * FROM `reseller` WHERE `email`='$email' limit 1";
	$data = $con->query($q);
	if ($data->num_rows > 0) {
        ...
        $message .= '<p style="color:#080;font-size:18px;"><a href="https:/'.$url.'/partner/security?se='.$rid.'&es='.$rid2.'">Click Here To Reset Your Link</a></p>';
        $message .= '</body></html>';
        ...
        if($sents){
            ...
            echo "<script>alert('Internal Error- Try Again Later'); window.location='../';</script>";
        }
